...
contract DOApp is Ownable {
   ...
   // boolean to check if production mode
    ...
    constructor(bool _isProductionMode, IDataStorage _dataStorage) Ownable() payable {
    ...
    function depositTokenA(uint _pairId, uint _amount) external {
        ...
        IDataStorage.TokenPairUserBalance memory userBalance = dataStorage.getTokenPairUserBalances(_pairId,msg.sender);
    ...
    function withdrawTokenA(uint _pairId, uint _amount) external {
        ...
        require( _amount <= dataStorage.getTokenPairUserBalances(_pairId,msg.sender).balanceA, "Amount to withdraw should be < your account balance");
        IDataStorage.TokenPair memory lPair = dataStorage.getTokenPair(_pairId);
    ...
    function withdrawTokenB(uint _pairId, uint _amount) external {
        require(_amount > 0, "Withdraw amount should be > 0");
        require( _amount <= dataStorage.getTokenPairUserBalances(_pairId,msg.sender).balanceB, "Amount to withdraw should be < your account balance");
        IDataStorage.TokenPair memory lPair = dataStorage.getTokenPair(_pairId);
      ...
      function executeDCA(uint _pairId) external returns (bool hasRemainingJobs){
        ...
        (segmentDCAToProcess) = getDCAJobs(lPair, IDataStorage.DCADelayEnum.Hourly, segmentDCAToProcess);
        ...
        if ((segmentDCAToProcess.segmentBuyEntries.length + segmentDCAToProcess.segmentSellEntries.length) 
            ...
            (segmentDCAToProcess) = getDCAJobs(lPair, IDataStorage.DCADelayEnum.Daily, segmentDCAToProcess);
        }

        if ((segmentDCAToProcess.segmentBuyEntries.length + segmentDCAToProcess.segmentSellEntries.length) 
            < MAX_DCA_JOB_PER_DCA_EXECUTION_CALL) {
            (segmentDCAToProcess) = getDCAJobs(lPair, IDataStorage.DCADelayEnum.Weekly, segmentDCAToProcess);
        }
...
function getDCAJobs(
    ...
    IDataStorage.SegmentDCAToProcess storage
){
    ...
    IDataStorage.SegmentDCAEntry[][2] memory segmentEntries = dataStorage.getDCASegment(_pair.pairID,moduloTokenPrice,_delay);
    ...
    if ((segmentEntries[0].length !=0 ) && segmentEntries[1].length !=0 ) {
                ...
                ((segmentEntries[0].length == cptBuy) && (segmentEntries[1].length == cptSell)))
        {
            ...
            // search for a buy entry
                (IDataStorage.SegmentDCAEntry memory segEntry, uint nextCptBuy) = getNextSegmentEntry(
                    segmentEntries[0],  
                    _delay, 
                    cptBuy, 
                    isBuy,
            ...
            // Search for a sell entry
                (IDataStorage.SegmentDCAEntry memory segEntry, uint nextCptSell) = getNextSegmentEntry(
                    segmentEntries[0],  
                    _delay, 
                    cptBuy, 
                    isBuy,
