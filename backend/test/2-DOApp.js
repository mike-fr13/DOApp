...
describe('DOApp', function () {
    ...
    describe("addTokenPair() tests", function () {
      ...
      it("Should revert if caller is not a owner", async function () {
        const {dOApp, tokenA, tokenB, mockChainLinkAggregatorV3, account1} 
          = await loadFixture(deployDOAppFixture);
        await expect(dOApp.connect(account1).addTokenPair(tokenA.address, tokenB.address, mockChainLinkAggregatorV3.address))
      ...
      it('Should revert when trying to add a pair with a null ChainLinkAggregatorV3 address', async function () {
        const {dOApp, tokenA, tokenB, mockChainLinkAggregatorV3, owner, account1, account2, account3, account4} 
          = await loadFixture(deployDOAppFixture);
        await expect(dOApp.addTokenPair(tokenA.address,tokenB.address, ADDRESS_0))
      ...
      it('Should revert when trying to add the same tokenPair with a revert order', async function () {
        const {dOApp, tokenA, tokenB, mockChainLinkAggregatorV3, owner, account1, account2, account3, account4} 
          = await loadFixture(deployDOAppFixture);
        await (dOApp.addTokenPair(tokenA.address, tokenB.address, mockChainLinkAggregatorV3.address))
        await expect(dOApp.addTokenPair(tokenB.address, tokenA.address, mockChainLinkAggregatorV3.address))
        .to.revertedWith("Token Pair Allready Defined")
      })
  ...
  describe('Token Deposit', function () {
    describe ('Token Deposit Error', function () {
    ...
    describe ('Token Deposit Success', function () {
      it('Should be able to deposit a specified amount of token A and check balance', async function () {
        const {dOApp, tokenA, account1, pairId} = await loadFixture(deployDOAppFixtureAddATokenPairDepositToken);
      ...
      it('Should Deposit a specified amount of token B', async function () {
        const {dOApp,  tokenB, account1, pairId} = await loadFixture(deployDOAppFixtureAddATokenPairDepositToken);
  ...
  describe('Token Withdraw', function () {
    describe ('Token Withdraw Error', function () {
